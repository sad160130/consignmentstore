...
def load_data():
    ...
    try:
        ...
        json_path = os.path.join('data', 'SEO_Optimized_Consignment_Stores_Sample_Dataset.json')
        if os.path.exists(json_path):
            ...
            excel_path = os.path.join('data', 'SEO_Optimized_Consignment_Stores_Sample_Dataset.xlsx')
...
# ... [Keep all your existing helper functions: format_description, process_data, get_nearby_cities] ...
...
# ... [Keep all your existing routes and context processors] ...

# Update the main execution block
if __name__ == '__main__':
    try:
        if df is None or df.empty:
            logger.error("Could not load data. Please check data files.")
            sys.exit(1)

        logger.info(f"Successfully loaded {len(df)} stores in {len(df['State'].unique())} states")
